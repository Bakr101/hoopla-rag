import argparse
from lib.hybrid_search import normalize, weighted_search, rrf_search, enhance_query
...
def main() -> None:
    ...
    normalize_parser = subparsers.add_parser(
        "normalize", help="Accepts a list of scores and prints the normalized scores"
    )
    ...
    weighted_search_parser.add_argument(
        "--alpha",
        type=float,
        help="Dynamically controls the weighting between Semantic Search and Keyword search.",
        default=DEFAULT_ALPHA,
        nargs="?",
    )
    weighted_search_parser.add_argument(
        "--limit", type=int, help="Limit the number of results", default=5, nargs="?"
    )
    ...
    rrf_search_parser.add_argument(
        "--k",
        type=int,
        help="Constant that controls how much more weight we give to higher-ranked results vs. lower-ranked ones Default is 60",
        default=60,
        nargs="?",
    )
    rrf_search_parser.add_argument(
    ...
    rrf_search_parser.add_argument(
        ...
        nargs="?",
        help=" LLM-based re-ranking for rrf search",
    )
    rrf_search_parser.add_argument(
        "--evaluate", action="store_true", help="Evaluate the search results"
    )
    enhance_query_parser = subparsers.add_parser(
        "enhance-query", help="Enhance a query"
    )
    enhance_query_parser.add_argument("query", type=str, help="Search query")
    enhance_query_parser.add_argument(
        "--enhance",
        type=str,
        ...
        case "rrf-search":
            ...
            for i, res in enumerate(result["results"], 1):
                ...
                if res["metadata"]["bm25_rank"] and res["metadata"]["semantic_rank"]:
                    print(
                        f"   BM25: {res['metadata']['bm25_rank']}, Semantic: {res['metadata']['semantic_rank']}"
                    )
